...
function Quotenot() {
  ...
  const getenquiryadd = async () => {
    ...
    if (res.status === 200) {
      setenquiryflwdata(res.data?.quotefollowup.filter((item) => item.response === "Not Intrested"));
      setSearchResults(res.data?.quotefollowup.filter((item) => item.response === "Not Intrested")); // Update the searchResults state with the full data
    }
  };
  let i = 0;
